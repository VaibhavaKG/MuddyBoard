...
export class MagicPaint {
  ...
  init(containerEl, onInteraction) {
    ...
    // 3. Slow canvas auto-clear loop (fades paint trails gradually)
  ...
  setupDrawingEvents() {
    const startDraw = (x, y) => {
      ...
      // Cycle to random brush on each new stroke for distraction-free fun
      const randomBrush =
        this.brushes[Math.floor(Math.random() * this.brushes.length)];
      this.brushType = randomBrush.id;
